...
class Board:
    ...
    def move_piece(self, coordinates, playing, opponent):
        ...
        if dif_dir['dif_x'] == 2:
            ...
            opponent.remove_piece(self.matrix[coordinates['to_y'] - dif_dir['dir_y']]
                                  [coordinates['to_x'] - dif_dir['dir_x']].get_piece())
            self.matrix[coordinates['to_y'] - dif_dir['dir_y']
                        ][coordinates['to_x'] - dif_dir['dir_x']].deallocate_piece()
            self.matrix[coordinates['to_y']][coordinates['to_x']].assign_piece(
                self.matrix[coordinates['from_y']][coordinates['from_x']].get_piece())
            self.matrix[coordinates['from_y']
                        ][coordinates['from_x']].deallocate_piece()
        ...
        if self.become_queen(playing, coordinates):
            self.matrix[coordinates['to_y']][coordinates['to_x']
                                             ].get_piece().convert_to_queen()
    ...
    def verify_forced_movements(self, playing):
        ...
        while index_row < self.size_m:
            ...
            while index_col < len(self.matrix[index_row]):
                ...
                forced = self.add_forced_movements(
                    test_coordinates, playing, forced)
